...
pub fn recite(start_bottles: u32, take_down: u32) -> String {
    ...
    for n in (end..=start).rev() {
        ...
        writeln!(&mut out, "{} green {} hanging on the wall,", head, bottle_word).unwrap();
        writeln!(&mut out, "{} green {} hanging on the wall,", head, bottle_word).unwrap();
        writeln!(&mut out, "And if one green bottle should accidentally fall,").unwrap();

        let remaining = if n > 1 { n - 1 } else { 0 };
        ...
        writeln!(&mut out, "There'll be {} green {} hanging on the wall.", rem_word, rem_bottle_word).unwrap();
